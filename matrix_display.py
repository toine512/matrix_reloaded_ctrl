...
#	Copyright © 2023  toine512 <me@toine512.fr>
#
...
class MatrixReloadedApp:
	...
	def __init__(self) -> None:
		...
		cli.add_argument("--log-level", action="store", choices=LOGGER._core.levels.keys(), help="Defaults to INFO. Messages level SUCCESS and higher are output to stderr. Level SUCCESS corresponds to successful events that are important to the user (good warnings), select WARNING if you want to only be notified of failure warnings. Setting log level to DEBUG is suggested while experimenting. TRACE level prints IRC communications, which will expose credentials!") # type: ignore
